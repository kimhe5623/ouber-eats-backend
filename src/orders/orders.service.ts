...
export class OrderService {
    ...
    try {
      ...
      for (const item of items) {
        ...
        for (const itemOption of item.options) {
          ...
          console.log(`dishFinal: ${dishFinalPrice}`);
        }
      ...
      console.log(`orderFinal: ${orderFinalPrice}`);
  ...
  async getOrders(
    ...
    try {
      ...
      if (user.role === UserRole.Client) {
        ...
        orders = restaurants.map(restaurant => restaurant.orders).flat(1); // Array 내부에서 Array를 바깥으로 빼내는 것
        if (status) {
  ...
  async editOrder(
    ...
    try {
      ...
      if (!canEdit) {
        return {
          ok: false,
          error: "You can't change this order status"
        }
      }
  ...
  async takeOrder(
    driver: User, { id: orderId }: TakeOrderInput
    ...
    if(order.driver) {
      return {
        ok: false,
        error: "This order has a driver already"
      }
    } 
    await this.orders.save([{
      id: orderId,
